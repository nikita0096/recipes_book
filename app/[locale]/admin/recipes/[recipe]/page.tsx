...
const Page = () => {
  ...
  const startEditing = (field: EditingField) => {
    ...
    const titleParsed = parseJson(recipe.title) as { ua: string; en: string };
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-amber-50 to-white dark:from-gray-900 dark:to-gray-800">
      ...
      <section className="relative h-[70vh] w-full overflow-hidden">
        {editingField === 'heroImage' ? (
          <div className="absolute inset-0 flex items-center justify-center bg-black/50 z-20">
            <div className="bg-white dark:bg-gray-800 rounded-2xl p-6 max-w-md w-full mx-4 shadow-xl">
              <h3 className="text-lg font-bold text-gray-900 dark:text-white mb-4">Edit Hero Image</h3>
              <input
                type="text"
                value={editedValues.heroImage}
                onChange={(e) => updateEditedValue('heroImage', e.target.value)}
                placeholder="Image URL"
                className="w-full px-3 py-2 text-gray-900 bg-gray-100 dark:bg-gray-700 dark:text-white rounded-lg border-2 border-amber-300 focus:outline-none focus:border-amber-500 mb-4"
              />
        ...
        {isEditing && editingField !== 'heroImage' && (
          <button
            onClick={() => startEditing('heroImage')}
            className="absolute top-3 left-3 z-10 flex items-center gap-2 px-3 py-2 bg-amber-100 dark:bg-amber-900/50 text-amber-700 dark:text-amber-300 font-medium rounded-xl hover:bg-amber-200 dark:hover:bg-amber-900 transition-colors shadow-md"
          >
            <CiEdit className="text-xl"/> <span>Change Image</span>
          </button>
        )}
        ...
        <div className="absolute bottom-0 left-0 right-0 p-8 md:p-16">
          <div className="max-w-4xl mx-auto">
              ...
              {editingField === 'title' ? (
                <div className="flex flex-col gap-2 mb-4 w-full max-w-xl">
                  <div className="flex items-center gap-2">
                    ...
                    <input
                      type="text"
                      value={editedValues.title.ua}
                      onChange={(e) => updateEditedValue('title', { ...editedValues.title, ua: e.target.value })}
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <input
                      type="text"
                      value={editedValues.title.en}
                      onChange={(e) => updateEditedValue('title', { ...editedValues.title, en: e.target.value })}
                      className="flex-1 px-3 py-2 text-lg font-bold text-gray-900 bg-white rounded-lg border-2 border-amber-300 focus:outline-none focus:border-amber-500"
                    />
                  </div>
            ...
            <div className="flex items-center gap-4 text-white/90">
              ...
              {isEditing && (
                <button className="bg-amber-100 dark:bg-amber-900/50 text-amber-700 dark:text-amber-300 rounded-xl hover:bg-amber-200 dark:hover:bg-amber-900 transition-colors shadow-md p-1">
                  <CiEdit className="text-2xl"/>
                </button>
              )}
              <span className="flex items-center gap-2">
        ...
        <div className='absolute top-3 right-3 flex flex-col gap-2'>
          ...
          <button
            className='flex flex-row items-center gap-2 px-4 py-2 bg-red-100 dark:bg-red-900/30 text-red-600 dark:text-red-400 font-medium rounded-xl hover:bg-red-200 dark:hover:bg-red-900/50 transition-colors shadow-md'
                ...
                {editingField === `step-${i}` && editedValues.step ? (
                  <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-lg overflow-hidden border-2 border-amber-400">
                    <div className="p-6">
                      ...
                      {/* Description UK */}
                      <div className="mb-4">
                        <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Description (UK)</label>
                        <textarea
